...
.bucle_bytes:
    ...
    mov r13, rcx        ; rcx = longitud del código nuevo
    shl r9,  cl         ; hacer espacio en el acumulador
    or  r9,  r11        ; meter código nuevo en los bits bajos
    add r10, r13        ; actualizar contador
...
.vaciar_bits:
    ...
    sub  rcx, 8
    mov  rax, r9
    shr  rax, cl            ; aislar byte superior
    mov  [rdi + rbx], al    ; escribir byte superior
    inc  rbx

    ; limpiar bits escritos del acumulador
    mov  rax, 1
    shl  rax, cl
    dec  rax
    and  r9, rax            ; conservar solo bits restantes
...
.finalizar_flujo:
    cmp  r10, 0
    je   .salir
    ; alinear bits sobrantes al bit más alto del byte
    mov  rcx, 8
    sub  rcx, r10
    shl  r9b, cl
    mov  [rdi + rbx], r9b
...
.escribir_par_rle:
    mov [rdi + rdx], r10b       ; Escribir byte de datos original
    mov [rdi + rdx + 1], al ; Escribir byte de conteo de repeticiones
    add rdx, 2
